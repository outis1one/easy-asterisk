...
#
# - ASTERISK: Intelligent ICE/STUN/TURN integration
#   * ICE support enabled ONLY for FQDN/internet calling (not LAN-only)
#   * pjsip.conf endpoints conditionally include ice_support based on setup
#   * rtp.conf icesupport enabled only when DOMAIN_NAME is configured
#   * Google STUN fallback made OPTIONAL (asks user)
#   * Can run with no STUN/TURN/ICE for LAN-only or VPN-only setups
#
...
SCRIPT_NAME="easy-asterisk-interactive-v1.27.sh"
...
transport=transport-tls"
    ...
    # ICE support only for FQDN/internet calling, not for LAN-only
    local ice_block=""
...
EOF
    ...
    if [[ -n "$DOMAIN_NAME" ]]; then
        ice_setting="icesupport=yes"
    else
        ice_setting="# icesupport disabled - LAN only mode"
    fi
...
local_net=$local_net"
    ...
    cat > "$conf_file" << EOF
; Easy Asterisk v1.27 (OPNsense/VLAN Ready)
; Generated: $(date)
[global]
type=global
...
show_main_menu() {
    clear
    print_header "Easy Asterisk v1.27"
...
submenu_install() {
    ...
    echo ""
    echo "  3) Server Only - Just Asterisk server"
    echo "     Optionally with FQDN/internet"
    echo ""
    echo "  4) Client Only - Just Baresip client"
    echo "     Connect to existing server"
    echo ""
    echo "  5) Uninstall"
    echo ""
    echo "  0) Back"
